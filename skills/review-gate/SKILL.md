...
The reports must be complete, belong to the same task and batch, use the same exact scope, come from different instances, preserve
different assigned `lens` values, link the counter report to the primary result, and use different evidence IDs. The reports may cite
