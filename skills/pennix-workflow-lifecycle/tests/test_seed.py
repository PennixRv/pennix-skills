...
class SeedTests(unittest.TestCase):
    ...
    def test_fresh_seed_writes_no_secret_to_output_or_config(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("继续当前 Codex 会话", result.stdout)
            self.assertIn("下一轮用户消息", result.stdout)
            self.assertIn("当前会话", result.stdout)
            self.assertIn("再下一轮用户消息", result.stdout)
            self.assertIn("不要新建会话", result.stdout)
