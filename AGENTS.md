...
- 工作流部署统一由 `pennix-workflow-bootstrap` guided Skill 编排；Trellis 项目初始化仍由原生 `trellis init` / `trellis workflow` 完成，组件适配脚本位于 bootstrap 的 `scripts/adapters/`，不复制初始化器或运行时协议副本。
- 不保存项目任务事实、会话、凭据、缓存、Plugin 物化目录或运行时状态。
- 安装副本由 bootstrap 调用内部 Skills deployment adapter 在用户明确请求时写入发现路径；不要在安装副本上形成
