...
将 Arch 远程 seed 改为可重入的 Pennix 运行时准备入口：一个新 Codex 会话的第一轮只安装 lifecycle bootstrap，安装器确认后同一会话的下一轮由该 bootstrap 完成 collection 并部署工作流，无需持久 source checkout。
...
- R3: Seed completion output contains one unambiguous new-session, two-turn
  prompt. Its first turn uses `$skill-installer` only for
  `pennix-workflow-lifecycle` at the managed destination; its next turn stays
  in the same session and contains no complete-collection child Skill list or
  source-checkout instruction.
- R4: The same-session lifecycle bootstrap must use the catalog's internal
...
- R5: The second conversation turn invokes `$pennix-workflow-lifecycle`
  directly. It recognizes the exact `bootstrap` collection state, completes it
  without another session, then continues deployment. Documentation must call
  this a one-session native-Skill availability boundary, not two deployment
  stages.
...
- [x] A fresh seed produces protected `config.toml` and `auth.json`, emits the
  first-turn lifecycle-bootstrap `$skill-installer` request and same-session
  next-turn lifecycle prompt, and never emits a complete-collection child Skill
