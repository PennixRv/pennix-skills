...
完成 seed 后，第一个新 Codex 会话使用系统 `$skill-installer` 只在最终 collection
destination 内安装 `pennix-workflow-lifecycle` bootstrap，然后结束。第二个新会话重新
发现该 Skill 并直接调用它；bootstrap 在同一第二会话补齐 collection 后继续部署。该运行时
发现边界不是第二个部署阶段，也不需要第三个会话。
...
- 两个文件都已存在时不读取 stdin 或 `/dev/tty`，仍确认 Codex 包并输出首会话
  指令。
...
第一个会话只安装 lifecycle bootstrap；第二个会话从 catalog 的
...
系统安装器不覆盖既有 destination，故第二个会话只安装尚未存在的 remaining Skills。
...
自动删除会越过当前 seed 的所有权边界。用户文档只给出新的两会话路径。
