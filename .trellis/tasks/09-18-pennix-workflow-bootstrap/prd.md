...
实现统一 guided bootstrap Skill，并将现有用户可见 setup Skill 收敛为内部 deployment adapters；保留原生组件边界、分阶段静态模板合同和可验证的 discover/install/upgrade/uninstall/verify。
...
- 将上述 setup 能力的确定性脚本、测试和必要 reference 迁移到 `skills/pennix-workflow-bootstrap/scripts/adapters/`，保留原生 owner、确认门禁和失败关闭。
- 新入口实现 `discover`、显式确认后的 `install`、`upgrade`、`uninstall` 和 `verify`；不生成计划、action receipt 或自动回滚层。
...
- seed 采用显式 source checkout + 受控脚本；生命周期命令逐组件确认；Codex 配置只允许白名单 key；catalog unknown 失败关闭；项目初始化不由 bootstrap 执行；提问后立即停 turn。
...
- 首期宿主严格限定为 Linux 上的 Arch Linux，覆盖原生 Arch Linux 与 WSL2；生命周期包操作优先使用已存在的 `yay`，没有时回退到 `paru`，最后使用 `pacman`；Stage 0 Codex 最小安装直接使用官方 `pacman`。
- 非 Arch、WSL1 或无法确认 WSL2 的环境可以执行只读 `discover`/`verify`，但所有生命周期写入必须失败关闭；bootstrap 不自动安装 AUR helper、不猜测包名、不把独立或 fork 组件强行改走包管理器。
- Stage 0 的 `latest` 只表示 Arch 官方同步源的当前候选版本，不写入 `component-versions.json`；Stage 1 继续使用唯一静态 catalog 做固定版本和漂移治理。
- Bootstrap 只管理系统组件；系统安装不隐式初始化项目，Trellis、CodeGraph、AOE 初始化必须使用各组件绑定明确 project root 的原生命令。
...
- [x] `discover`/`verify` 为只读且不暴露敏感值；未显式确认的 install/upgrade/uninstall 拒绝执行；卸载只处理精确 bootstrap-owned 内容。
